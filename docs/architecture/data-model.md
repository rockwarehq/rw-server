...
The Prisma schema is split across ~24 domain files in `packages/db/schema/`. This page gives you the mental model: the entity hierarchy, the conventions every domain follows, and the metric/OEE design. For per-domain crow's-foot ER diagrams and the standing list of data-model audit findings, see [Data Model ERD & Audit](./erd.md).
...
- **Stock only changes through the stock book** (ADR-0016): save the record (a made part, scrap, an order, a count, a material ledger row), then post it with `postSources` from `@rw/services/stock/post` in the same save. Never write `StockBalance` directly, and never edit a `StockMovement`; cancel it with `reverseSources` and post again.
