...
model GraphProperty {
  ...
  name          String
  resolverType  String   // 'tag' | 'entity' | 'expr' | 'window' | 'rollup'
...
The flush loop dispatches on `resolverType`. In v1, both `expr` and `window` produce computed output (`tag` and `entity` properties get their values from subscriptions). Build the dispatch as an explicit switch (or registry) so future resolver kinds slot in cleanly.
...
Counter as a distinct kind — a tumbling window with `aggregation: 'count'` covers parts-per-shift and event tallies. A general counter-with-arbitrary-reset (rising-edge, cron schedules) is deferred.
...
For tumbling: if `now > bucketEnd`, fast-forward: emit the last bucket with `quality: 'stale'`, advance buckets up to the current time, emit any intermediate buckets as `count: 0, quality: 'bad'` (gaps), then open the current-time bucket
...
Tumbling on restart:
Load state. `bucketStart`, `bucketEnd`, accumulator fields.
If `now < bucketEnd`: bucket is still open. Schedule close timer for `bucketEnd - now`. Resume accepting input.
If `now >= bucketEnd`: bucket closed during downtime. Emit it with `quality: 'stale'` (we know it's incomplete). Advance buckets:
If only one bucket was missed: open it as the current bucket
If multiple were missed: emit each missed bucket as `count: 0, quality: 'bad'` (gap), then open the current one
The emitted "missed" buckets are visible to dependent expressions and dashboards — operators see the gap rather than a phantom continuous trend.
EWMA on restart:
