...
// Resolve rollup membership from the domain model (spec §18.3): for each rollup
// property, validate its relation against the catalog, query the parent entity's
// children via that relation, and map each child to its childProperty ID. Returns
// synthetic child -> rollup edges so the dependency graph orders and dirties
// rollups like any other dependency. Not persisted — Station.workcenterId (etc.)
// is the source of truth; this is rebuilt each boot from Postgres.
