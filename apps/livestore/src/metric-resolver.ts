...
// Handler for `metric` resolver properties (parallel to TagResolver). Feeds each
// leaf (e.g. a station's shift_goodItems) from NATS: a publisher — a simulator
// now, the metrics worker later — writes the current MetricBucket value to
// metrics.<entityId>.<granularity>.<metricKey>; this commits it to the matching
// property, which then rolls up. Swap the simulator for the worker; nothing here
// changes.
