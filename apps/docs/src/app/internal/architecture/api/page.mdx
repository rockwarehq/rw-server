...
5. Graph-definition publisher, entity-event publisher, command bus — the NATS/JetStream adapters under `src/nats/` (app-owned composition roots, see [ADR-0004](/internal/adrs/0004-app-owned-composition-roots))
...
- **Service results, not exceptions**: services return `{ data }` or `{ error, code }`. RPC handlers map codes to `ORPCError`s through the single shared mapper in `src/rpc/errors.ts` (`throwServiceError` / `unwrap`); divergent historical mappings are pinned as overrides — see [ADR-0003](/internal/adrs/0003-service-error-contract). Unhandled throws hit the oRPC interceptor in `server.ts`, get logged, and return 500.
- **Database access**: writes always go through `@rw/services`; a short, named list of RPC routers may run complex read-only reporting queries against Prisma directly — see [ADR-0002](/internal/adrs/0002-database-access-boundary).
