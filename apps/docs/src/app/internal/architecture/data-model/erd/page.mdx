export const metadata = {
  title: 'Data Model ERD & Audit',
  description:
    'Crow’s-foot entity-relationship diagrams for every Prisma domain, plus a structured list of data-model audit findings.',
}

# Data Model ERD & Audit

Crow's-foot ER diagrams for all ~100 models in `packages/db/schema/`, one diagram per domain, followed by [audit findings](#audit-findings) collected while mapping the schema. Built for reviewing the data model as a whole — not just what the database enforces, but what it silently doesn't. {{ className: 'lead' }}
...
No global middleware enforces filtering; every query must remember. See the standing `TODO-soft-delete-audit.md` at the repo root for the per-callsite audit.
