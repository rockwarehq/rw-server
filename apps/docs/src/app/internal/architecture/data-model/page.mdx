...
- **Blob versioning**: config that affects historical interpretation is snapshotted — `StationBlob`, `JobBlob`, `ToolBlob` — and cycles reference the blob ids in effect at the time. Historical queries must read the blob, not current config.
