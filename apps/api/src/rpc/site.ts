...
const updateSettingsInputSchema = z.object({
  ...
  settings: z.object({
    ...
    // Deprecated compatibility preference; never grants production authority.
...
export const get = userOrDisplayRequired.input(idInputSchema).handler(async ({ input, context }) => {
  const scope = grant(await authorizeReferenceRead(context.iam, { scope: { kind: "site", siteId: input.id } }));
...
export const tree = userOrDisplayRequired.input(treeInputSchema).handler(async ({ input, context }) => {
  if (context.iam.principal === Principal.DISPLAY) {
    ...
    const scope = grant(
      await authorizeReferenceRead(context.iam, {
  ...
  if (input.siteId) {
    const scope = grant(await authorizeReferenceRead(context.iam, { scope: { kind: "site", siteId: input.siteId } }));
...
export const getSettings = authRequired.input(idInputSchema).handler(async ({ input, context }) => {
  grant(await authorizeReferenceRead(context.iam, { scope: { kind: "site", siteId: input.id } }));
...
export const updateSettings = authRequired.input(updateSettingsInputSchema).handler(async ({ input, context }) => {
  if (input.settings.orderAutoComplete !== undefined) {
    grant(
      await authorize(context.iam, { permission: "configuration:write", scope: { kind: "site", siteId: input.id } }),
    );
  }
  if (input.settings.baseWorkcenterAccess !== undefined || Object.keys(input.settings).length === 0) {
    grant(await authorize(context.iam, { permission: "plant:admin", scope: { kind: "site", siteId: input.id } }));
  }
...
export const update = authRequired.input(updateInputSchema).handler(async ({ input, context }) => {
  ...
  for (const permission of site.siteUpdatePermissions(updateData)) {
    grant(await authorize(context.iam, { permission, scope: { kind: "site", siteId: id } }));
  }
  const result = await site.update(id, updateData, context.iam.workspaceId);
  if (result.error !== undefined) throwServiceError(result);
  return result.data;
});
