...
export const get = authRequired.input(idInputSchema).handler(async ({ input, context }) => {
  grant(await authorize(context.iam, { tier: "VIEW", scope: { kind: "statusReason", id: input.id } }));
...
export const update = authRequired.input(updateInputSchema).handler(async ({ input, context }) => {
  grant(await authorize(context.iam, { tier: "MANAGE", scope: { kind: "statusReason", id: input.id } }));
