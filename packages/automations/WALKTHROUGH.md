...
How to wire `@rw/automations` into a new app, from nothing to a firing event. This is the
...
"dependencies": { "@rw/automations": "workspace:*" }
```
```jsonc
// your app's tsconfig.json — so project references build it
"references": [{ "path": "../../packages/automations" }]
```

Everything below assembles into one call: `createAutomationFramework(config)`. The config has five
...
```ts
import type { ActionSchema, EventSchema, EventType } from "@rw/automations";
...
by `automation.action.type`, so adding actions never touches the engine.

```ts
import { type ActionHandler, createActionRegistry } from "@rw/automations";
...
schemas must have an entry here** — `createAutomationFramework` validates this at construction time
...
```ts
import { statelessContextBuilder, type ContextBuilder, type EventType } from "@rw/automations";
...
Implement `AutomationStore` against your database. For dev, an in-memory map is enough — the engine
only needs these five methods:

```ts
import type { Automation, AutomationStore } from "@rw/automations";

function createMemoryStore(): AutomationStore {
...
> The store persists automation **definitions** only — it does not drive evaluation. After any
> `upsert`/`remove` you MUST call `engine.reload()` (see step 6/7). See the `AutomationStore` doc
> comment in [`store.ts`](./src/store.ts) for the multi-instance (Redis pub/sub) plan.

## 5. Assemble — `createAutomationFramework(config)`
...
`createAutomationFramework` calls `engine.reload()` for you, so the framework is ready to fire as
soon as it returns. Expose `fw` as a singleton (e.g. `getAutomationFramework()`).

## 6. Author an automation, then raise an event

Automations are authored through your own UI/RPC, which writes to the store and reloads:
...
- Edit anything inside `@rw/automations`.
