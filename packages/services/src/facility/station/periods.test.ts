...
describe.skipIf(!process.env.DATABASE_URL)("shift period splits", () => {
  ...
  test("a reason stamps its planned flag on every piece of the block; the period can override it", async () => {
    ...
    expect(await assignDowntimeReason(open.id, planned.id, { applyToBlock: true })).toMatchObject({
      success: true,
      updatedCount: 2,
    });
    ...
    expect(await assignDowntimeReason(open.id, planned.id, { applyToBlock: true, isPlannedDown: false })).toMatchObject(
      { success: true },
    );
    expect(await flags()).toEqual([
      [true, false],
