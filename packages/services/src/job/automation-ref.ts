 ...
 * without a name there's nothing the picker can render. Workspace-scoped via `site.workspaceId`.
 */
export function createJobsAutomationRef(workspaceId: string): RefSource {
  return {
    key: "jobs",
    async list(_ctx) {
      const rows = await prisma.job.findMany({
        where: { site: { workspaceId }, deletedAt: null, currentBlobId: { not: null } },
