 ...
 * sendAlert@v2 lands — or surface a separate `getEmployeeEmail(workspaceId, id)` if the consumer
 * doesn't need the name + email together.
 */
...
/**
 * `employees` ref source backed by Postgres. Lists every active employee in the workspace whose
 * current version has a name, ordered alphabetically by display name. `{ id, label }` shape
 * matches the other ref sources so the editor renders uniformly.
 */
export function createEmployeesAutomationRef(workspaceId: string): RefSource {
  return {
    key: "employees",
    async list(_ctx) {
      const employees = await employeesInWorkspace(workspaceId);
...
/**
 * Resolve a single employee by id, scoped to the workspace. Returns undefined if the employee
 * doesn't exist, isn't in this workspace, or has no current version (no name to render).
 */
export async function getEmployeeById(workspaceId: string, id: string): Promise<ResolvedEmployee | undefined> {
  const employee = await prisma.employee.findFirst({
    where: { id, workspaceId, versionId: { not: null } },
...
/** Every active employee in the workspace with a current version. Stable name-ordered for picker UX. */
async function employeesInWorkspace(workspaceId: string): Promise<ResolvedEmployee[]> {
  const rows = await prisma.employee.findMany({
    where: { workspaceId, versionId: { not: null } },
    select: { id: true, version: { select: { firstName: true, lastName: true } } },
  });
  const out = rows
